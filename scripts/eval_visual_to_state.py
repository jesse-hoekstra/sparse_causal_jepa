"""Visual-to-state evaluation: ``python scripts/eval_visual_to_state.py <run_dir>``.
...
Two alignments are used, deliberately: ``pred_loss``/``constraint_loss`` under
the TRAINING assignment (Eq. 99) so tau_2 is calibrated against the quantity the
dual actually sees, and ``mcc``/``shd`` under the EVALUATION assignment
(Eqs. 137-138), which is blind to learned parameters and true masses.

Writes ``metrics.json`` in the shared key namespace, so the pipeline reads
``constraint_loss`` for tau exactly as the state-to-state pipeline does.
"""
