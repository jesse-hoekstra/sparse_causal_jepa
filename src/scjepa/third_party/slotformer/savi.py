...
# scjepa patch (see PROVENANCE.md): nerv dependency replaced by local shim.
from .nerv_shim import BaseModel, deconv_out_shape, conv_norm_act, deconv_norm_act
...
class StoSAVi(BaseModel):
    ...
    def __init__(
        ...
        self._build_encoder()
        # scjepa patch (see PROVENANCE.md): decoder only exists on the
        # reconstruction path, which this project never trains (D7 — no
        # reconstruction objective). Skip building it when unused.
        if self.loss_dict['use_post_recon_loss']:
            self._build_decoder()
        # scjepa patch (see PROVENANCE.md): the slot predictor is only invoked
        # from the second frame onward; single-frame use (D9 target encoder)
        # passes pred_dict=None to skip building it.
    ...
    def _build_loss(self):
        ...
        # scjepa patch (see PROVENANCE.md): upstream asserted this flag True;
        # encoder-only use (D7) runs with it False.
    ...
    def _reset_rnn(self):
        # scjepa patch (see PROVENANCE.md): no-op when no predictor was built.
        if self.pred_dict is not None:
