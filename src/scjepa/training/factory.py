...
REGIMES = ("state_to_state", "visual_to_visual")
"""The two observation contracts, named context-to-target.
...
) -> StateToStateModel | VisualToVisualModel:
...
def build_dataset(data_cfg: DictConfig, seed_offset: int = 0) -> Dataset[dict[str, Tensor]]:
    ...
    return BounceDataset(
        ...
        # A visual episode is ~18 MB. Caching held-out frames scales to tens of
        # GB for final evaluation; the deterministic generator can serve them
        # without retaining the rendered tensors between batches.
        cache=bool(data_cfg.get("cache", False)) and not (seed_offset and data_cfg.get("render")),
    )
